...
type SketchPlan = { width: number; height: number; title: string; views?: readonly { id: string; label: string }[]; components: readonly { id: string; label: string; color: string; note?: string; nodeIds?: readonly string[]; points: readonly { x: number; y: number }[]; views?: Readonly<Record<string, readonly { x: number; y: number }[]>> }[]; annotations: readonly { label: string; x: number; y: number }[] };
...
function SketchReview({ draft, pending, onSave, onFeedback, onApprove }: { draft: ModelingDraft; pending: boolean; onSave: (iteration: SketchIteration, strokes: readonly SketchStroke[]) => void; onFeedback: (iteration: SketchIteration, prompt: string, strokes: readonly SketchStroke[]) => void; onApprove: (iteration: SketchIteration) => void }) {
  ...
  useEffect(() => { setStrokes(iteration?.markup ?? []); setRedoStrokes([]); setFeedback(""); setViewId(iteration?.plan.views?.[0]?.id ?? "front"); }, [iteration?.id]);
  ...
  const begin = (event: ReactPointerEvent<SVGSVGElement>) => { if (iteration.status !== "proposed") return; const start = pointerPoint(event); if (!start) return; if (eraser) { const nearest = strokes.map((stroke) => ({ stroke, distance: Math.min(...stroke.points.map((item) => Math.hypot(item.x - start.x, item.y - start.y))) })).sort((a, b) => a.distance - b.distance)[0]; if (nearest?.distance < .04) { setRedoStrokes((current) => [...current, nearest.stroke]); setStrokes((current) => current.filter((item) => item.id !== nearest.stroke.id)); } return; } const target = event.target as Element; const shape = target.closest("[data-sketch-component]"); const componentId = shape?.getAttribute("data-sketch-component") ?? null; const nodeId = shape?.getAttribute("data-sketch-node") ?? null; const pen = PEN_COLORS.find((item) => item.semantic === semantic) ?? PEN_COLORS[0]; const id = `stroke-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`; activeStroke.current = id; setRedoStrokes([]); event.currentTarget.setPointerCapture?.(event.pointerId); setStrokes((current) => [...current, { id, semantic, color: pen.color, width: 5, note: "", componentId, nodeId, viewId, points: [start] }]); };
  ...
  return <SketchReviewPanel>
    ...
    <SketchCanvas viewBox={`0 0 ${iteration.plan.width} ${iteration.plan.height}`} role="img" aria-label={`${iteration.plan.title} ${viewId} 주석 캔버스`} onPointerDown={begin} onPointerMove={draw} onPointerUp={end} onPointerCancel={end} onLostPointerCapture={end}>
      {iteration.plan.components.map((component) => <g key={component.id}>
        <polygon data-sketch-component={component.id} data-sketch-node={component.nodeIds?.[0] ?? ""} points={(component.views?.[viewId] ?? component.points).map((point) => `${point.x},${point.y}`).join(" ")} fill={component.color} fillOpacity="0.18" stroke={component.color} strokeWidth="3" />
        <text x={(component.views?.[viewId] ?? component.points)[0]?.x ?? 0} y={((component.views?.[viewId] ?? component.points)[0]?.y ?? 20) - 10} fill="currentColor" fontSize="18">{component.label}</text>
      </g>)}
    ...
    <SketchComponentLegend>{iteration.plan.components.map((component) => <li key={component.id}>{component.label} · {component.nodeIds?.length ?? 0}개 graph node</li>)}</SketchComponentLegend>
...
function ModelingCatalogRegion({ definition }: { definition: ProductPageDefinition }) {
  ...
  const [selectedChildRefIds, setSelectedChildRefIds] = useState<readonly string[]>([]);
  ...
  const refreshProductModels = useCallback(async () => {
    ...
    if (!response.ok || !body.ok) throw new Error(body.error ?? "조립 파일 목록을 불러오지 못했습니다.");
  ...
  useEffect(() => {
    if (!activeParentModelId) { setActiveParentTree(null); setSelectedChildRefIds([]); return; }
    setSelectedChildRefIds([]);
  ...
  const bindSku = async (productModel: ProductModel, skuId: string | null): Promise<ProductModel | null> => {
    ...
    try {
      ...
      return body.model;
    } catch (requestError) { setModelListError(requestError instanceof Error ? requestError.message : String(requestError)); await refreshProductModels().catch(() => undefined); return null; }
    finally { setBindingPendingId(null); }
  };

  const unlinkSkuForDeletion = async () => {
    if (!deleteTarget) return;
    const modelAfterUnlink = await bindSku(deleteTarget, null);
    if (modelAfterUnlink) setDeleteTarget(modelAfterUnlink);
  };
  ...
  const archiveParent = async () => {
    ...
    try {
      ...
      if (!response.ok || !body.ok) throw new Error(body.error ?? "조립 파일을 삭제하지 못했습니다.");
  ...
  const restoreParent = async (item: ProductModel) => {
    ...
    try {
      ...
      const body = await response.json() as { ok?: boolean; error?: string };
      if (!response.ok || !body.ok) throw new Error(body.error ?? "조립 파일을 복원하지 못했습니다.");
  ...
  const beginAddChild = (tree: ProductModelTree) => {
    setProductName(tree.name); setComponentInput("새 구성요소"); setEditTarget({ mode: "add-child", rootModelId: tree.id, baseRootRevisionId: activeParentTree?.selectedRevision.id ?? tree.selectedRevision.id, label: `${tree.name} + OpenAI × Blender · 구성요소 추가` }); setDraft(null); setPreviewModel("");
  };
  ...
  const removeChildAsset = async (tree: ProductModelTree, childRefId: string) => {
    ...
    try {
      ...
      setSelectedChildRefIds((current) => current.filter((id) => id !== childRefId)); await refreshProductModels(); await refreshParentTree(tree.id);
      return true;
  ...
  const toggleChildActionSelection = (childRefId: string) => setSelectedChildRefIds((current) => current.includes(childRefId)
    ? current.filter((id) => id !== childRefId)
    : [...current, childRefId]);
  const selectChildActionTarget = (childRefId: string) => setSelectedChildRefIds((current) => current.includes(childRefId) ? current : [...current, childRefId]);
    ...
    <ModelingStudio>
      <Surface className={CLASS.modelingForm}>
        <form onSubmit={submit}>
          <ModelingWorkspaceIntro><Label>OPENAI × BLENDER</Label><Atom as="h2">{editTarget ? editTarget.label : "새 조립 파일 생성"}</Atom><Copy>{editTarget ? "저장된 기준 조립 파일과 선택하지 않은 구성요소는 그대로 유지합니다. 이 작업은 보완 전용입니다." : "새 조립 파일과 최초 구성요소를 생성합니다. 첫 Blender 결과가 검증되기 전에는 제품 자산 라이브러리에 저장되지 않습니다."}</Copy></ModelingWorkspaceIntro>
    ...
    {buildInProgress ? <BuildProgressPanel><ProcessProgressPanel>
      ...
      <AssetLibraryGrid aria-label="조립 파일과 SKU 연결 카드 목록">
        {productModels.length === 0 ? <AssetLibraryCard><AssetEmptyState><Label>저장된 조립 파일이 없습니다.</Label><Copy>새 조립 파일의 첫 Blender 결과가 검증되면 이 목록에 추가됩니다.</Copy></AssetEmptyState></AssetLibraryCard> : productModels.map((productModel) => <AssetLibraryCard key={productModel.id} selected={productModel.id === activeParentModelId}>
          ...
          <AssetNodeActions><ActionButton className={CLASS.modelingAction} intent="edit" onClick={() => setEditingName({ id: productModel.id, value: productModel.name, revision: productModel.revision, kind: "parent" })}>이름 수정</ActionButton><ActionButton className={CLASS.modelingAction} intent="edit" onClick={() => void startParentRefine(productModel)}>전체 보완</ActionButton><ActionButton className={CLASS.modelingAction} intent="danger" disabled={assetActionPending === productModel.id} onClick={() => setDeleteTarget(productModel)}>삭제</ActionButton></AssetNodeActions>
          {deleteTarget?.id === productModel.id ? <DestructiveActionGate><Label>조립 파일 삭제</Label><Copy>{deleteTarget.name}은 복구 가능한 보관 상태로 전환됩니다. 게시 artifact와 과거 리비전은 보존됩니다.{deleteTarget.linkedSkuId ? " 홈 연결을 보호하려면 SKU 연결을 먼저 해제하세요." : ""}</Copy><AssetNodeActions><ActionButton className={CLASS.modelingAction} onClick={() => setDeleteTarget(null)}>취소</ActionButton>{deleteTarget.linkedSkuId ? <ActionButton className={CLASS.modelingAction} intent="hide" disabled={bindingPendingId === deleteTarget.id} onClick={() => void unlinkSkuForDeletion()}>SKU 연결 해제</ActionButton> : null}<ActionButton className={CLASS.modelingAction} intent="danger" disabled={Boolean(deleteTarget.linkedSkuId) || assetActionPending === deleteTarget.id} onClick={() => void archiveParent()}>삭제 확인</ActionButton></AssetNodeActions></DestructiveActionGate> : null}
        </AssetLibraryCard>)}
      </AssetLibraryGrid>
      ...
      {activeParentTree ? <ModelingLibraryTree aria-label={`${activeParentTree.name} 조립 파일과 구성요소`}>
        <Atom className={CLASS.modelingParentToolbar}><AssetIdentity><strong>{activeParentTree.name}</strong><small>최신 r{activeParentTree.currentRevision?.ordinal ?? 0} · 게시 {activeParentTree.publishedRevision ? `r${activeParentTree.publishedRevision.ordinal}` : "없음"} · {activeParentTree.status}</small></AssetIdentity><AssetNodeActions><ActionButton className={CLASS.modelingAction} intent="publish" disabled={!activeParentTree.linkedSkuId || !activeParentTree.selectedRevision.assetPath || assetActionPending === activeParentTree.id} onClick={() => void publishAssembly(activeParentTree)}>홈에 표시</ActionButton><ActionButton className={CLASS.modelingAction} intent="edit" onClick={() => beginAssetRefine(activeParentTree)}>조립 파일 보완</ActionButton><ActionButton className={CLASS.modelingAction} intent="add" onClick={() => beginAddChild(activeParentTree)}>구성요소 추가</ActionButton></AssetNodeActions></Atom>
        {flatAssetNodes.length ? <AssetLibraryGrid aria-label={`${activeParentTree.name} 구성요소 카드 목록`}>{flatAssetNodes.map((node) => {
          const item = node.child.model; const visible = node.child.visible !== false; const selectedForAction = selectedChildRefIds.includes(node.child.id);
          const canMutate = node.depth === 0 && activeParentTree.selectedRevision.id === activeParentTree.currentRevision?.id;
          return <AssetLibraryCard key={node.path} selected={selectedForAction}>
            <SelectionCardControl aria-label={`${item.name} ${selectedForAction ? "작업 대상 선택 해제" : "작업 대상으로 선택"}`} aria-pressed={selectedForAction} onClick={() => toggleChildActionSelection(node.child.id)}>
            ...
            <AssetNodeActions aria-label={`${item.name} 작업`}>{canMutate ? <ActionButton className={CLASS.modelingAction} intent={visible ? "hide" : "show"} disabled={assetActionPending === node.child.id} onClick={() => void setChildVisibility(activeParentTree, node, !visible)}>{visible ? "숨기기" : "표시"}</ActionButton> : null}<ActionButton className={CLASS.modelingAction} intent="edit" onClick={() => { selectChildActionTarget(node.child.id); setEditingName({ id: item.id, value: item.name, revision: item.revision, kind: "child" }); }}>이름 수정</ActionButton>{canMutate ? <><ActionButton className={CLASS.modelingAction} intent="edit" onClick={() => { selectChildActionTarget(node.child.id); beginAssetRefine(activeParentTree, node.child); }}>OpenAI × Blender로 구성요소 보완</ActionButton><ActionButton className={CLASS.modelingAction} intent="danger" disabled={assetActionPending === node.child.id} onClick={() => { selectChildActionTarget(node.child.id); setChildDeleteTarget({ tree: activeParentTree, childRefId: node.child.id, childName: item.name }); }}>삭제</ActionButton></> : null}</AssetNodeActions>
            {childDeleteTarget?.childRefId === node.child.id ? <DestructiveActionGate><Label>구성요소 삭제</Label><Copy>{childDeleteTarget.childName}을(를) 현재 조립 파일에서 제외하고 보관합니다. 과거 리비전과 게시 artifact는 보존됩니다.</Copy><AssetNodeActions><ActionButton className={CLASS.modelingAction} onClick={() => setChildDeleteTarget(null)}>취소</ActionButton><ActionButton className={CLASS.modelingAction} intent="danger" disabled={assetActionPending === node.child.id} onClick={() => void confirmChildDelete()}>삭제 확인</ActionButton></AssetNodeActions></DestructiveActionGate> : null}
          </AssetLibraryCard>;
