...
function repairTargetKey(error) {
  ...
  return /^(?:graph_repair_required|unsupported_operation):\s*([^\.\s:]+)\./.exec(message)?.[1] ?? null;
}
...
async function repairGraphComponent({ raw, componentKey, model, payload, evidenceManifest, imageInputs, runtime, diagnostic = null }) {
  ...
  const repaired = await responseJson({
    ...
    instructions: "Repair exactly one safe declarative ModelingGraph component fragment. Return exactly one component with its same immutable componentKey. A rib, shell, transform, mate, or boolean cut must reference a real preceding generating solid through inputKeys. A radial rib pattern has exactly two inputs in order: the base solid and one rib feature; the rib has exactly the base solid as its only input. Every rib must declare positive numeric heightMm, spacingMm, and depthMm; its count is either positive numeric count or supplied by its following pattern. Never use a whole body/cap as a pattern seed, never feed a pattern into a revolve, and do not give revolve/extrude/primitive/loft/sweep features any inputs. A sweep must declare a non-repeated 3-D path plus either a positive radiusMm or a closed local XY profile. Every revolve that creates a B-Rep solid must have a closed, non-zero-area section with at least four ordered profile points; never use a three-point decorative stroke as a solid cutter or ring. A brep_solid must contain at least one revolve, extrude, primitive, loft, or sweep generating feature and exactly one terminal B-Rep root: connect its body, ribs, rings, and cuts by explicit boolean/pattern feature inputs; never leave independent roots or duplicate an identical root feature. A surface_decal or surface_artwork must declare its hostComponentKey, primary evidence artworkImageId/crop, planar/cylindrical/uv projection, positive physical dimensionsMm.x/y, and transform.translationMm.z; it is an image-backed visual component, not a generic B-Rep. Every Boolean union must have a non-zero contact volume or face overlap; a separate requested component must not be duplicated as a disconnected decorative ring inside this component. If evidence does not establish physical contact, remove the redundant feature and record the uncertainty rather than inventing a bridge. A cavity cutter profile must remain strictly inside the corresponding outer profile at every shared z value; if a wall thickness is not image- or user-supported, request it as an unresolved parameter instead of cutting through the external wall. A cavity cut must explicitly leave the intended wall/roof thickness or explicitly open at its datum face; it must not merely touch a closed outer face. Add only an image-supported, allowed generating feature if it is required for that connection. Preserve host/material intent. Never write code, paths, URLs, HTML, or executable expressions. Do not create unrelated components or alter the product.",
...
export async function analyseDraft(payload, imageInputs, runtime = {}) {
  ...
  // Each constrained iteration changes only a narrow graph scope.  Reuse the
  // unchanged components' already validated OCCT reports/preview triangles,
  // keyed inside the preflight module by their reachable graph closure.
  // This is a performance cache, never a geometry substitute: every changed
  ...
  if (process.env.NET30_MODELING_DRAFT_FIXTURE !== "true") {
    ...
    compiledClosureDatum = fitCompiledClosureDatum(canonical.graph, brepPreflight, approvedDimensions);
    ...
    for (let iteration = 0; iteration < contourFitLimit; iteration += 1) {
      ...
      // Image fitting may improve a local curve but must never drift the
      // approved overall width/depth. Apply the same B-Rep graph datum before
      ...
      compiledContourFits.push({ iteration: iteration + 1, applied: true, adjustments: candidate.adjustments, approvedRadialDatum: candidateDatum.adjustments, before: compiledContour, after: candidateContour, relativeImprovement });
  ...
  const graphContour = compareAxisymmetricContour(canonical.graph, imageEvidence, primaryImageId);
  const contour = compareBrepAssemblyContour(brepPreflight, imageEvidence, primaryImageId) ?? compareBrepAxisymmetricContour(brepPreflight, imageEvidence, primaryImageId) ?? graphContour;
