...
export const draftPayloadSchema = z.object({ version: z.union([z.literal("net30.modeling-draft.v3"), z.literal("net30.modeling-draft.v4"), z.literal("net30.modeling-draft.v5")]).optional(), model: z.string().trim().max(160).optional(), product: z.object({ source: z.enum(["existing", "new"]), productId: z.string().trim().max(160).optional(), name: z.string().trim().min(1).max(160).optional() }).superRefine((value, ctx) => { if (value.source === "new" && !value.name) ctx.addIssue({ code: "custom", message: "새 제품명은 필수입니다." }); if (value.source === "existing" && !value.productId) ctx.addIssue({ code: "custom", message: "기존 제품 ID는 필수입니다." }); }), parentModelId: z.string().trim().min(1).max(160).optional(), componentInput: z.string().max(2000).optional(), revisionBaseRefs: z.record(z.string(), assetRefSchema).default({}), assemblyAssetRefs: z.array(assetRefSchema).max(60).default([]), prompt: z.string().trim().min(1).max(4000), imageIds: z.array(z.string().uuid()).max(4).default([]), skuId: z.string().trim().min(1).max(160).optional() }).transform((payload) => ({ ...payload, version: "net30.modeling-draft.v5", requestedComponents: normaliseComponentInput(payload.componentInput ?? "제품 본체") }));
