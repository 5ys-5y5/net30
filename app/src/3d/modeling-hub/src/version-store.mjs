...
export function createVersionStore(assetRoot) {
  ...
  const empty = { version: 2, showcase: null, components: Object.fromEntries(COMPONENTS.map((component) => [component, []])) };
  ...
  async function list(component) { const manifest = await load(); return manifest.components[component] ?? []; }
  async function register({ component, sourcePath, jobId, contractHash, summary, parentVersionId = null, approvedSpec = null, bounds = null, axis = "z-up", transform = null, interfaces = [] }) {
    ...
    const entry = { id, component, ordinal, jobId, contractHash, summary, parentVersionId, approvedSpec, bounds, axis, transform, interfaces, createdAt: new Date().toISOString(), assetPath: `/api/modeling/components/${component}/versions/${id}/artifact` };
