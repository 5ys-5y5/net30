...
export const modelingPayloadSchema = z.object({ version: z.union([z.literal("net30.modeling-job.v2"),z.literal("net30.modeling-job.v3")]).optional(), component: componentSchema.optional(), components: z.array(componentSchema).min(1).max(30).optional(), componentPrompts: z.record(z.string(), z.string().trim().max(2000)).default({}), parentVersionId: z.record(z.string(), z.string().min(1).max(160)).default({}), prompt: z.string().trim().min(1).max(4000), settings: settingsSchema.default({}), dimensionOverrides: settingsSchema.default({}), model: z.string().trim().max(160).optional(), imageIds: z.array(z.string().uuid()).max(4).default([]), specFileIds: z.array(z.string().uuid()).max(8).default([]), compiledSpec: z.unknown().optional(), graphHash: z.string().nullable().optional(), approvedDraft: z.unknown().optional(), quality: z.enum(["standard", "high"]).default("high") }).transform((value) => ({ ...value, version: value.version??"net30.modeling-job.v2", components: [...new Set(value.components ?? (value.component ? [value.component] : []))] }));
...
async function cadExports(spec, requestPath, cadDir) {
  ...
  for (let index = 0; index < cadComponents.length; index += 2) {
    await Promise.all(cadComponents.slice(index, index + 2).map(async (component) => {
      ...
      await fs.writeFile(file, JSON.stringify({ component, graphComponent, graphNodes, contract: spec.contract, output }));
...
export async function executeBlenderModeling(rawPayload, { assetRoot, jobId = `job-${Date.now()}`, spec, imageInputs = [], onProgress = () => undefined } = {}) {
  ...
  await fs.writeFile(requestPath, `${JSON.stringify({ payload, spec: modelingSpec, imageInputs, paths: { jobDir, assemblyGlb, componentDir, cadDir }, jobId }, null, 2)}\n`);
