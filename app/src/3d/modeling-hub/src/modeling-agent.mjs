...
export async function runModelingJob(payload, { jobId, imageInputs = [], onProgress = () => undefined } = {}) {
  ...
  const approvedSpec = payload.compiledSpec;
  const analysis = approvedSpec ? { contract: approvedSpec.contract, source: "approved-draft", model: null } : await createAssemblyContract(payload, imageInputs); const hash = contractHash(analysis.contract);
  ...
  const specs = approvedSpec?.components ?? await Promise.all(payload.components.map((component) => createComponentSpec({ payload, contract: analysis.contract, component, imageInputs, model: analysis.model })));
  ...
  const report = { jobId, status, contractHash: hash, manufacturingCandidate: !needsThreadEvidence, requiredReview: needsThreadEvidence ? analysis.contract.unresolved : [], components: specs.map((item) => ({ component: item.component ?? item.componentInstanceId, componentInstanceId: item.componentInstanceId, displayName: item.displayName, contractHash: item.contractHash })), generatedAt: new Date().toISOString() };
