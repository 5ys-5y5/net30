...
export function qualityGates({ graphHash, contour = null, landmarks = null, dimensions = null, brep = null, step = null, evidenceComplete = false }) {
  ...
  const gates = [
    { id: "silhouette_iou", state: contour?.iou >= .97 ? "pass" : contour ? "fail" : "not_measured", value: value(contour?.iou), threshold: .97, message: contour?.source === "occt_brep_assembly_tessellation" ? "Compiled OCCT B-Rep assembly contour against calibrated primary-image silhouette" : contour?.source === "occt_brep_tessellation" ? "Compiled OCCT B-Rep contour against calibrated primary-image silhouette" : "Fitted graph contour against calibrated primary-image silhouette" },
    { id: "contour_rms_mm", state: contour?.rmsMm <= .35 ? "pass" : contour ? "fail" : "not_measured", value: value(contour?.rmsMm), threshold: .35, message: contour?.source === "occt_brep_assembly_tessellation" ? "Compiled OCCT B-Rep assembly contour RMS against calibrated measurements" : contour?.source === "occt_brep_tessellation" ? "Compiled OCCT B-Rep contour RMS against calibrated measurements" : "Fitted graph contour RMS against calibrated measurements" },
    { id: "hausdorff95_mm", state: contour?.hausdorff95Mm <= .75 ? "pass" : contour ? "fail" : "not_measured", value: value(contour?.hausdorff95Mm), threshold: .75, message: contour?.source === "occt_brep_assembly_tessellation" ? "Compiled OCCT B-Rep assembly contour 95th percentile distance" : contour?.source === "occt_brep_tessellation" ? "Compiled OCCT B-Rep contour 95th percentile distance" : "Fitted graph contour 95th percentile distance" },
