...
const cropPlacedArtwork = fixtureGraphOutput({ product: { name: "crop 부착 인쇄" }, prompt: "front print", requestedComponents: ["body", "front print"], imageIds: ["image-1"] });
cropPlacedArtwork.components[1].features[0].parameters.transform = null;
const cropPlacedCanonical = canonicalizeGraph(cropPlacedArtwork, ["body", "front print"], ["image-1"]);
const cropPlacedNode = cropPlacedCanonical.graph.nodes.find((node) => node.operation === "surface_decal");
assert.ok(Number.isFinite(cropPlacedNode.parameters.transform?.translationMm?.z), "an evidence crop with a host derives an initial physical placement before the artwork fitter runs");
...
const closureCurve = closureBase.parameters.profile.map(({ xMm, zMm }) => ({ xMm, zMm }));
closureBase.parameters.profile = null;
closureBase.parameters.curveSegments = closureCurve.slice(1).map((point, index) => ({ kind: "line", points: [closureCurve[index], point], periodic: false }));
